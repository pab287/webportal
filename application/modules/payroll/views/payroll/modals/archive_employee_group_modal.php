<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Archive Payroll Employee Group</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="frmArchiveEmployeeGroup" method="post" action="<?php echo site_url("payroll/employee/archive_payroll_employee_group"); ?>">
      <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
      <input type="hidden" name="id" value="<?= $data->id; ?>" />
      <div class="modal-body">
        <h5>Are you sure you want to archive this payroll employee group entry?</h5>
