...
public function getEmployeeNightDiffList(){
    ...
    public function getEmployeeAutoOvertimeList(){
        $rowCount = 0;
        $rowData = array();
        $resultset = array();
        $post = $this->input->post();
        ...
        $rowData = $this->employeeAutoOvertime($search, $limit, $offset, $sortBy, $sortOrder, $filters);
        $rowCount = $this->employeeAutoOvertimeCount($search, $filters);
    ...
    private function tableAutoOvertime($search=null, $filterFields = [], $filters = []){
        $this->db->select("emp.id as employee_id, emp.idno, UPPER(CONCAT(emp.lastname,
            CASE WHEN UPPER(TRIM(emp.suffix)) != 'N/A' AND
                UPPER(TRIM(emp.suffix !='NONE')) AND emp.suffix !='' AND
                emp.suffix IS NOT NULL THEN CONCAT(' ', emp.suffix) ELSE ''
            ...
            CASE WHEN UPPER(TRIM(emp.middlename)) != 'N/A' AND UPPER(TRIM(emp.middlename)) != 'NONE' AND
                ...
                THEN CONCAT(' ', SUBSTR(emp.middlename, 1, 1), '.') ELSE ''
            END)) as employee_name, UPPER(cmp.code) as company_code, UPPER(IFNULL(pos.name, emp.position)) as position,
            auto.allow_auto_overtime, auto.id, IFNULL(auto.allow_auto_overtime, '0') as allow_auto_overtime,
            IFNULL(auto.last_updated_at, auto.created_at) as last_updated_at,
            IF(auto.last_updated_by = 0,
                CONCAT(UPPER(TRIM(cemp.firstname)),
                CASE WHEN UPPER(TRIM(cemp.middlename)) != 'N/A' AND UPPER(TRIM(cemp.middlename)) != 'NONE' AND
                        TRIM(cemp.middlename) !='' AND cemp.middlename IS NOT NULL
                    THEN CONCAT(' ', UPPER(SUBSTR(cemp.middlename, 1, 1)), '. ') ELSE ''
        ...
        $this->db->join($this->companyTable . " cmp", "cmp.id = emp.company_id", "left");
        $this->db->join($this->positionTable . " pos", "pos.id = emp.position OR pos.name = emp.position", "left");
        $this->db->join($this->tbl_ps_auto_overtime . " auto", "auto.employee_id = emp.id", "left");
    ...
    private function employeeAutoOvertime($search = null, $limit = 10, $offset = 0, $sortBy=[], $sortOrder=[], $filters=[]){
        $filterFields = array("emp.firstname", "emp.lastname", "emp.idno");
        $queryDb = $this->tableAutoOvertime($search, $filterFields, $filters);
        if($limit != -1){ $queryDb->limit($limit, $offset); }
        if (isset($sortOrder)) {
            $i = $sortOrder[0]['column'];
            $queryDb->order_by($sortBy[$i]['data'], $sortOrder[0]['dir']);
    ...
    private function employeeAutoOvertimeCount($search, $filters){
        $filterFields = array("emp.firstname", "emp.lastname", "emp.idno");
        $queryDb = $this->tableAutoOvertime($search, $filterFields, $filters);
        $query = $queryDb->get();
        return $query->num_rows();
    ...
    public function updateAutoOvertimeStatus(){
        $post = $this->input->post();
        $resultset = array();
        if(isset($post) && $post){
            $resultset = $this->insertUpdateAutoOvertime($post);
        }else{
            $resultset["response"] = false;
            $resultset["toastr_msg"] = "Failed to update Auto Overtime Status, no post data found.";
        }
        return $resultset;
    }

    protected function insertUpdateAutoOvertime($data = []){
        $response = false;
        $type = isset($data["id"]) && $data["id"] ? "update" : "insert";
        $data["allow_auto_overtime"] = isset($data["allow_auto_overtime"]) && intval($data["allow_auto_overtime"]) === 1 ? 0 : 1;
        if(isset($data["id"]) && $data["id"]){
            $where = array("id"=>$data["id"]);
            unset($data["id"], $data["employee_id"]);
            ...
            $data["last_updated_at"] = date("Y-m-d H:i:s");
            $data["last_updated_by"] = $this->core_layout->getCurrentEmployeeId();
            $response = $this->db->update($this->tbl_ps_auto_overtime, $data, $where);
        }else{
            unset($data["id"]);
            $data["created_at"] = date("Y-m-d H:i:s");
            $data["created_by"] = $this->core_layout->getCurrentEmployeeId();
            $response = $this->db->insert($this->tbl_ps_auto_overtime, $data);
        }

        if($response && $this->db->affected_rows() > 0){
            return array("response" => true, "toastr_msg" => "Auto Overtime has been ".($type == "insert" ? "added" : "updated")." successfully.");
        }else{
            return array("response" => false, "toastr_msg" => "Failed to ".($type == "insert" ? "add" : "update")." Auto Overtime.");
        }
    }
    ...
    public function approveEmployeeGroupTransfer(){
        $post = $this->input->post();
    ...
    private function addEmployeesToNewGroup($groupId, $employeeIds, $currentEmployeeId, $now){
        ...
        if ($updated && $this->db->affected_rows() > 0) {
            $this->logEmployeeGroupTransfer($employeeIds, 'approved', $groupDescription);
            return true;
        }
