<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Allowance_m extends CI_Model {
        ...
        private function masterfile_list($search=null, $limit = 10, $offset = 0, $sortBy, $sortOrder) {
            $filterFields = array("a.code", "a.allowance_name");
            $this->db->select("a.*");
            $this->db->from('payroll.allowance a');
        ...
        function save(){
            ...
            if($getCode == 0){
                $data = array(
                    "code" => $post['code'],
                    "allowance_name" => $post['name'],
                    "created_by" => $this->user_data['emp_id'],
                    "created_at" => $date     
                );

                $query = $this->db->insert('payroll.allowance', $data);

                if($query){
                    $result['toastr_msg']="Allowance has been created";
                    $result['state'] = true;
                }else{
        ...
        function update($id){
            ...
            if($getCode == 0){
                ...
                if($id){
                    $this->db->where('allowance.id', $id);
                    $query = $this->db->update('payroll.allowance', $data);

                    if($query){
                        $result['toastr_msg']="Allowance has been updated";
                        $result['state'] = true;
                    }else{
        ...
        function delete($id) {
            ...
            if($query){
                $result['toastr_msg']="Allowance has been removed";
                $result['state'] = true;
            }else{
        ...
        function restore($id) {
            ...
            if($query){
                $result['toastr_msg']="Allowance has been restored";
                $result['state'] = true;
            }else{
                $result['toastr_msg']="Error removing data";
                $result['state'] = false;
