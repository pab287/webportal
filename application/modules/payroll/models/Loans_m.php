<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Loans_m extends CI_Model {
        ...
        private function masterfile_list($search=null, $limit = 10, $offset = 0, $sortBy, $sortOrder) {
            $filterFields = array("a.code", "a.loan_name");
            $this->db->select("a.*");
            $this->db->from('payroll.loans a');
        ...
        function save(){
            ...
            $post = $this->input->post();
            $loanType = (isset($post["loan_type"]) && $post["loan_type"])? intval($post["loan_type"]): 0;
            $loanClass = (isset($post["loan_class"]) && $post["loan_class"])? intval($post["loan_class"]): 0;
            $getCode = $this->getCode($post['code']);
            $has_ref = (isset($post['has_ref']))?$post['has_ref']:0;
            if($getCode == 0){
                $data = array(
                    "code" => $post['code'],
                    "loan_name" => $post['name'],
                ...
                $query = $this->db->insert('payroll.loans', $data);

                if($query){
                    $result['toastr_msg']="Loan has been created";
                    $result['state'] = true;
                }else{
        ...
        function update($id){
            ...
            if($getCode == 0){
                ...
                if($id){
                    ...
                    $query = $this->db->update('payroll.loans', $data);

                    if($query){
                        $result['toastr_msg']="Loan has been updated";
                        $result['state'] = true;
                    }else{
        ...
        function delete($id) {
            ...
            if($query){
                $result['toastr_msg']="Loan has been removed";
                $result['state'] = true;
            }else{
        ...
        function restore($id) {
            ...
            if($query){
                $result['toastr_msg']="Loan has been restored";
                $result['state'] = true;
            }else{
                $result['toastr_msg']="Error removing data";
                $result['state'] = false;
