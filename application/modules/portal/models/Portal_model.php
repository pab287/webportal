...
class Portal_model extends CI_Model{
    ...
    public function getPayslip() {
        ...
        if ($id) {
            $ps_id = $this->db->select('id')->order_by('pay_date', 'desc')->limit(1)->get_where('payroll.payroll_sheet', array('emp_id' => $id, 'posted' => 1, "printed_payslip" => 1, 'is_bonus' => 0))->row();
            ...
            if ($ps_id) {
                ...
                $qTemp = $this->db->get_where("payroll.payroll_sheet as ps", array("ps.id"=>$ps_id->id, "ps.posted"=>1, "ps.printed_payslip" => 1, 'ps.is_bonus' => 0));
