<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Calendar extends MY_Controller
    {
        ...
        public function add_participants($id){
            ...
            $this->core_layout->addJs('global/plugins/swal/sweetalert2.all.min.js', true);
            $this->core_layout->addJs("js/hris/calendar/add_participants_script.js", true,$data);
            $this->core_layout->addCss("css/hris/calendar.css", true);
            $this->load->view("core/templates/header");
            $this->load->view("masterfile/calendar/calendar_of_events/participants_page");
            $this->load->view("core/templates/footer");
        }
        ...
        public function confirm_participant(){
            $data = $this->holiday->confirmParticipant();
            $this->output->set_content_type('json')->set_output(json_encode($data));
        }

        public function decline_participant(){
            $data = $this->holiday->declineParticipant();
            $this->output->set_content_type('json')->set_output(json_encode($data));
        }

        public function archive_participant(){
            $data = $this->holiday->archiveParticipant();
            $this->output->set_content_type('json')->set_output(json_encode($data));
        }
