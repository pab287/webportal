...
class Events_model extends MX_Controller {
    ...
    private function getEventsData($limit, $offset, $sortBy, $sortOrder, $search , $year,$is_archived){
        $filterFields = array("a.event_title", "a.description", "a.event_venue", "a.event_from", "a.event_to","a.company_array","a.department_array","a.events_by");
        $havingFields = array("speaker_positions", "speaker_names", "speaker_companies");
        ...
        $this->db->where("YEAR(a.event_from)", $year);
        ...
        $havingParts = [];
        foreach ($havingFields as $field) {
            $havingParts[] = "$field LIKE '%" . $this->db->escape_like_str($search) . "%'";
        }
        if ($havingParts) {
            $this->db->having(implode(' OR ', $havingParts));
        }
