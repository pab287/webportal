...
class Hydra_billing_login_m extends Dbase{
		...
		$sth->bindParam(':pssword', $password);
   		...
   		if ($count > 0) {
			...
			array_push($response['info_array'], $list);
   			...
   			$this->saveLogs("success", "sign in", $row['emp_id'], "[Mobile] User sign in");
   		} else {
   			$list['status'] = 'failed';
			array_push($response['info_array'], $list);
   		}
	...
	public function user_logout(){
		...
		if ($this->saveLogs("success", "sign out", $user_id, "[Mobile] User sign out")) {
			echo "success";
		} else {
			echo "failed";
		}
    ...
    public function test_only(){
    	$test = $this->findAccntNoDash($this->conn(), "20102722");
    	if ($test) {
    		echo $test;
    	} else {
    		echo "string";
	...
	private function findAccntNoDash($conn, $meterno){
		$meterno_raw = str_replace("-", "", $meterno);
		$sth = $conn->prepare("SELECT meterno FROM hydra_billing.accounts ORDER BY `id` ASC");
		$sth->execute();
		if ($sth->rowCount() > 0) {
			while ($row = $sth->fetch(PDO::FETCH_ASSOC)) {
		 		$meterno_ = $row['meterno'];
		 		if ($meterno_raw == str_replace("-", "", $meterno_)) {
		 			return $meterno_;
		 		}
			}
		}
