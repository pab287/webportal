<?php
    ...
    protected function getBaseUrl() {
        ...
        if ($host === 'localhost' || strpos($host, '192.168.') === 0) {
            $path = '/conyxph74/login/forgotpassword/changepassword';
        } else {
            $path = '/web/login/forgotpassword/changepassword';
        }
    ...
    public function forgotPassword(){
        ...
        $result = $this->checkEmailIfExist($email);
        $conn = $this->conn();
        $getEmployee = null;
        $baseUrl = $this->getBaseUrl();
        $key = bin2hex(random_bytes(35));
        $key = substr($key, 0, 69);
        ...
        $stmt->execute();
        $getUser = $stmt->fetch(PDO::FETCH_OBJ);
        ...
        if ($getUser) {
            $stmt2 = $conn->prepare("
                ...
                LIMIT 1
            ");
            $stmt2->bindParam(':emp_id', $getUser->emp_id);
            $stmt2->execute();
            $getEmployee = $stmt2->fetch(PDO::FETCH_OBJ);
        }

        $data = [
                "firstname" => $getEmployee->firstname ?? '',
                "username" => $getUser->username ?? '',
                "rawchangepasslink" => $baseUrl . "?key=" . $key,
            ];
        ...
        if($result){
            $msg = "If the email you provided is associated with an account, you will receive a password reset link shortly.";
    ...
    protected function checkEmailIfExist($email = null){
        $status = false;
        if($email){
            $conn = $this->conn("gccmaster");
            $sth = $conn->prepare("SELECT email FROM gccmaster.tblusers WHERE email = :email");
