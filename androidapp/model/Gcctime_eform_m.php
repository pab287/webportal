<?php
    class Gcctime_eform_m extends Dbase{        
        use Logs_maker;
    ...
    public function gcctimeEformLogin() {
        ...
        if ($emp_data) {
            ...
            return json_encode(["status" => false, "msg" => $check]);
        }
    ...
    public function isSuspended() {
        ...
        try {
            ...
            return json_encode([
                ...
                "msg" => $data
                    ? ($data['is_suspended'] ? "Your account is suspended,\nplease contact your system administrator." : "Your account is active.")
                    : "Employee ID not found."
            ]);
        } catch (PDOException $e) {
            return json_encode(["status" => false, "msg" => "Database error: " . $e->getMessage()]);
        }
    }
    ...
    public function isSignout() {
        ...
        try {
            ...
            if ($stmt->execute()) {
                ...
                return json_encode(["status" => true, "msg" => "Successfully signed out."]);
            }
    
            throw new Exception("Database update error");
        } catch (Exception $e) {
            $this->saveLogs("error", "sign out", $emp_id, "[Mobile] Sign out failed - " . $e->getMessage());
            return json_encode(["status" => false, "msg" => "Failed to sign out: " . $e->getMessage()]);
        }
    }
            ...
            if ($latitude !== null && $longitude !== null) {
                ...
                if ($exists > 0) {
                    ...
                    if ($data->execute()) {
                        ...
                        $this->get_supExeManag_contact($emp_id, $remarks, $timeStatus, $geo_status, $date_time, $bio_num, $latitude, $longitude);
                    } else {
                        $list['status'] = false;
                        $failedCount++;
                    }
    ...
    public function timeLog() {
        ...
        if ($missing) {
            return json_encode(["status" => 0, "msg" => "Missing or empty field(s): " . implode(", ", $missing)]);
        }
    
        $bio = $_POST['biometric_id'];
        $emp = $_POST['emp_id'];
        $status = $_POST['time_status'] ?? '';
        ...
        if (!isset($coords['latitude'], $coords['longitude'])) {
            ...
            return json_encode(["status" => 0, "msg" => "Coordinates not found."]);
        }
        ...
        if ($geo_status === 1 && empty($remarks)) {
            return json_encode(["status" => 0, "msg" => "Missing or empty field(s): remarks"]);
        }
        ...
        if (!$sites_id) {
            ...
            return json_encode(["status" => 0, "msg" => "No Site Location Found."]);
        }
    
        if ($interval && $max_time >= $time) {
            $this->saveLogs("error", $logs_action, $emp, "[Mobile] Re-logged within 1-minute interval");
            return json_encode($this->user_logs($bio, $date, $time, 4));
        }
        ...
        if (!$insertedID) {
            ...
            return json_encode(["status" => 3, "msg" => "Error in saving $logs_action"]);
        }
    
        $this->get_supExeManag_contact($emp, $remarks, $this->timeStatusString($status), $geo_status, $dateTime, $bio, $latitude, $longitude);
        $this->log($bio, $longitude, $latitude);
        $this->saveLogs("success", $logs_action, $emp, "[Mobile] $logs_action");
        return json_encode($this->user_logs($bio, $date, $time, $resp, $insertedID));
    }
    ...
    function get_supExeManag_contact($emp_id, $remarks, $time_status, $geo_status, $date_time, $bio_num, $latitude, $longitude) {
