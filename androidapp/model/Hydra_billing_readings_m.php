...
class Hydra_billing_readings_m extends Dbase{
	...
	public function get_customer_details(){
		...
		$current_date = '2025-02-15'; // This is a fixed date, not generated by date() function
		...
		try {
			...
			if ($sth->rowCount() > 0) {
				...
				$balance_last_bill = $this->check_unpaid_balance($account_id);
	...
	public function check_unpaid_balance($account_id) {
		$list_bill_not_paid = $this->getBillNotPaid($account_id);
		$total_charges = 0;
		foreach ($list_bill_not_paid as $bill_id) {
			$bill = $this->getBillTotalCharges($bill_id);
			$total_charges += $bill;
		}
	...
	public function getBillNotPaid($account_id) {
		$conn = $this->conn();
		$res = array();
		$sth = $conn->prepare("SELECT b.id
	...
	public function getBillTotalCharges($bill_id) {
		$conn = $this->conn();
		...
		try {
			$sth = $conn->prepare("SELECT b.id, b.total_charges, b.due_date, b.billing_to
									FROM hydra_billing.bills b
									WHERE b.id = :bill_id");
			$sth->bindParam(':bill_id', $bill_id);
			$sth->execute();
			$row = $sth->fetch(PDO::FETCH_ASSOC);
			if ($row) {
			...
			return $total_charges + $overdue_charges;
		} catch (PDOException $e) {
			return $e->getMessage();
		}
	}
		...
		try {
			if (isset($_POST['reading_id']) && $_POST['reading_id'] != "" && $_POST['reading_id'] != 0 && isset($_POST['user_id']) && $_POST['user_id'] != "" && $_POST['user_id'] != 0) {
				...
				if (!$rate) {
					...
					if (isset($_POST['reading_id']) && $_POST['reading_id'] != "" && $_POST['reading_id'] != 0 && isset($_POST['user_id']) && $_POST['user_id'] != "" && $_POST['user_id'] != 0) {
						$sth = $conn->prepare("INSERT INTO hydra_billing.bills(
								`billing_from`, 
								`billing_to`, 
								`due_date`, 
								`ref_no`, 
