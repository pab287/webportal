...
class Hydra_billing_readings_m extends Dbase{
	...
	public function get_customer_details(){
		...
		$current_date = '2025-01-17'; // This is a fixed date, not generated by date() function
	...
	public function testGetBilling($reading_id, $account_id) {
		return $this->getBilling($reading_id, $account_id);
	}
	...
	private function getBilling($reading_id,$account_id){
		...
		// $total_amount_due = ($row["total_charges"] + $balance_last_bill['total_penalty'] + $reconnectionFee) - $over_payment;
	...
	public function test_compute_balance_last_bill($account_id) {
		return $this->computeBalanceLastBill($account_id);
	}
	...
	private function computeOverPayment($account_id){
		try {
			...
			$res = array();
			// $sth = $conn->prepare("SELECT SUM(received_amount - ) as total_received_amount,
			// 					   SUM(balance_covered) as balance_covered,
			// 					   (SELECT net_payment FROM hydra_billing.payments WHERE bill_id = p.bill_id AND is_archive = 0 ORDER BY id ASC LIMIT 1) as net_payment
			// 					   FROM hydra_billing.payments as p
			// 					   WHERE account_id = :account_id AND is_archive = 0");

			// $sth = $conn->prepare("SELECT SUM(received_amount - (SELECT net_payment FROM hydra_billing.payments WHERE bill_id = p.bill_id AND is_archive = 0 ORDER BY id ASC LIMIT 1)) as total_received_amount,
			// 					   SUM(balance_covered) as balance_covered,
			// 					   FROM hydra_billing.payments as p
			// 					   WHERE account_id = :account_id AND is_archive = 0");

			$sth = $conn->prepare("SELECT 
										SUM(received_amount - COALESCE((
											SELECT net_payment 
											FROM hydra_billing.payments 
											WHERE bill_id = p.bill_id AND is_archive = 0 
			...
			$result = $sth->fetch(PDO::FETCH_ASSOC);
			// return $result;
			$res['total_received_amount'] = $result['total_received_amount'];
			$res['balance_covered'] = $result['balance_covered'];
			// $res['net_payment'] = $result['net_payment'];
			// $recieved_net_calc = $result['total_received_amount'] - $result['net_payment'];
			// $total = $result['total_received_amount'] - $result['balance_covered'];
			return $res;
