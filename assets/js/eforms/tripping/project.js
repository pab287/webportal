var dtProjects = $('#table-projects').DataTable( {
    serverSide: true,
    processing: true,
    ajax: baseUrl("eforms/tripping/get_project_collection"),
    searching: true,
    columns: [
        { data: "code"},
        { data: "description"},
        { data: "name"},
        { data: "status",render: function (data) {return renderStatusHtml(data)}},
        { data: null},
    ],
    columnDefs: [
      { targets: [4] , width: "5%"},
      {
          data: null,
          defaultContent: "",
          targets: -1,
          orderable: false,
          render: function ( data, type, row, meta ) { return itemDatatableActions(row.id); },
      }
    ]
  });

function renderStatusHtml(data){
    switch(data){
        case "1":
            return '<div class="m-badge m-badge--accent m-badge--wide" role="alert"><strong>Active</strong></div>';
        break;
        default:
            return '<div class="m-badge m-badge--metal text-white m-badge--wide" role="alert"><strong>Cancelled</strong></div>';
        break;
    }
}

function itemDatatableActions($id){
  if($id){
		var _actionButton ="";
			_actionButton += "<a class='btn btn-default m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill btnEditItem' data-id='"+$id+"'><i class='la la-pencil-square'></i></a>";
		return _actionButton;
	}else{ return false; }
}

$("#companySelect").select2({
    placeholder: 'SELECT Company',
    width: '100%',
    ajax: {
      url: baseUrl("eforms/tripping/get_company"),
      global: false,
      processResults: function (data) {
        return data;
      }
  
    }
});

$.validate({
    form: '#frmNewProject',
    lang: 'en',
    onSuccess: function (form) {
        $.ajax({
          url: $("#frmNewProject").attr('action'),
          type: "POST",
          data: $('#frmNewProject').serialize(),
          dataType: "JSON",
          success: function (data) {
              if (data.status) {
              toastr.success(data.msg, "Notification");
              dtProjects.ajax.reload();
              $('#frmNewProject')[0].reset();
              } else {
                toastr.error(data.msg, "Notification");
              }
          }
        });
        return false;
    },
  });

  $("#table-projects").on("click",".btnEditItem",function(){
    var id = $(this).attr("data-id");

    $("#companySelectU").select2({
      placeholder: 'SELECT Company',
      width: '100%',
      ajax: {
        url: baseUrl("eforms/tripping/get_company"),
        global: false,
        processResults: function (data) {
          return data;
        }
    
      }
  });
  
    $.ajax({
        url: baseUrl("eforms/tripping/get_project_data"),
        type: "POST",
        data: {id: id, csrf_token :_csrf_hash,},
        success: function(response){
            $("#frmEditProject input[name=id]").val(response.id);
            $("#frmEditProject input[name=code]").val(response.code);
            $("#frmEditProject input[name=name]").val(response.name);
            var newOption = new Option(response.description, response.id, true, true);
            $('#frmEditProject #companySelectU').append(newOption).trigger('change');
        }
    });
  
    $("#m_editProject").modal("show");
  });

  $.validate({
    form: '#frmEditProject',
    lang: 'en',
    onSuccess: function (form) {
        $.ajax({
          url: $("#frmEditProject").attr('action'),
          type: "POST",
          data: $('#frmEditProject').serialize(),
          dataType: "JSON",
          success: function (data) {
              if (data.status) {
              toastr.success(data.msg, "Notification");
              dtProjects.ajax.reload();
              
              $("#m_newProject").modal("hide");
              } else {
                toastr.error(data.msg, "Notification");
              }
          }
        });
        return false;
    },
  });