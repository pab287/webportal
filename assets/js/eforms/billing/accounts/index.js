...
var tblAccounts = $("#table-accounts").DataTable({
   ...
   aaSorting: [],
   ajax: {
        url: baseUrl("eforms/billing/get_datatable_request/"),
        type: "post",
        global: false,
        dataType: "json",
        data: function(d){
   ...
   searching: true,
   columns: [
       { data: "name", width: "10%", render: function (data, type, row) {
                // return "<strong style='color: #525252;'>"+data+"</strong>";
                return `<a target="_blank" href="`+ baseUrl('eforms/billing/edit_account/') + row.id + `" ><strong style="color: #525252;">`+ data +`</strong></a>`;
            }
        },
...
function itemDatatableActions(row){
	if(row){
        ...
        var tempHtml = "---";
        var tempActions = [];
        var currentActions = ["edit", "replace_meter", "reset_meter"];
        ...
        tempHtml = `<div class="dropdown">
            ...
            $.each(tempActions, function(ii, vv){
                switch(vv){
                    ...
                    break;
                    case "replace_meter":
                    tempHtml += `<a class="dropdown-item " data-toggle='modal' data-target='#delete_modal' href="javascript:void(0);" onclick='showMeterReplaceModal(`+ row.id +`,\"`+ row.meterno +`\",`+`\"` + row.name + `\",` + row.previous_reading + `)'><i class="la la-tachometer"></i> Meter Replacement</a>`;
                    break;
                }
            });
            tempHtml += `</div></div>`;
...
function showMeterReplaceModal(id,meter,name, pr){
    ...
    $('#m_meter_r input[name=previous_reading]').val(pr);
...
$('.reset-meter').on('change', function() {
    if($('.reset-meter:checked').filter(function() {return $.trim(this.value) !== '';}).length > 0) {
        $('#m_meter_r input[name=new_meterno]').prop('readonly', true);
        $('#m_meter_r input[name=new_meterno]').val($('#m_meter_r input[name=old_meterno]').val());
    } else {
        $('#m_meter_r input[name=new_meterno]').prop('readonly', false);
