...
var tblAccounts = $("#table-accounts").DataTable({
   ...
   aaSorting: [],
   ajax: {
        url: baseUrl("eforms/billing/get_datatable_request_reconnection/"),
        type: "post",
        global: false,
        dataType: "json",
        data: function(d){
...
function reconnectModal(){
  $(".chckBox").each(function(){
    ...
    if(trig){
      paid_accounts.push($(this).attr("value"));
    }
  });
  if(paid_accounts.length > 0){
    $("#modal_reconnect").modal('show');
  }else{
    toastr.error("Please select an account.", "No account selected.");
  }
}

function reconnectSelect(){
  $.ajax({
      url: baseUrl("eforms/billing/reconnect_selected"),
      type: "POST",
      data:{paid_accounts: paid_accounts, csrf_token: _csrf_hash},
