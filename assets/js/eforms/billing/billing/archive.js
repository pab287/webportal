...
const tblBilling = $("#table-billing-archive").DataTable({
   ...
   aaSorting: [],
   ajax: {
        url: baseUrl("eforms/billing/get_billing_archive_collection/"),
        type: "post",
        global: false,
        dataType: "json",
        data: function(d){
...
$("#cb-select-all").click(function () {
  $('#table-billing tbody input[type="checkbox"]').prop('checked', this.checked);
});

$("#table-billing").on("click", "tbody input[type='checkbox']", function () {
  const allCheckboxes = $("#table-billing tbody input[type='checkbox']").length;
  const checkedCheckboxes = $("#table-billing tbody input[type='checkbox']:checked").length;
