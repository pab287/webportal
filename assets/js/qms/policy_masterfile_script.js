...
$(document).ready( function(){
    ...
    $('#timeline-query-builder').queryBuilder({
        'bt-tooltip-errors': { delay: 100 },
        filters: [
            { id: 'a.id', label: 'ID #', type: 'integer' },
            { id: 'a.title', label: 'Title', type: 'string' },
    ...
    $("#timeline-query-builder_group_0").addClass("col-12");
});
...
$('#timeline-query-builder-btn').on('click', function () {
    var result = $('#timeline-query-builder').queryBuilder('getSQL');

    if (!$.isEmptyObject(result)) {
        query_builder = result;
        vmTab1.getTimeline();
        $("#modal-timeline-query-builder").modal("hide");
    }
});
...
function clear_timeline_query_builder() {
    $('#timeline-query-builder').queryBuilder('reset');
    query_builder = null;
    vmTab1.getTimeline();
}
...
function getCategory(){
    $.ajax({
        url: baseUrl('qms/masterfile/get_categories'),
        dataType: "json",
        success: function(json){
...
var vmTab1 = new Vue({
    ...
    methods: {
        ...
        }, setDepartmentSelect2(targetElement, modalContent = null, select2Destroy = false){
            const currentTarget = $(targetElement);
            if(typeof modalContent != 'undefined' && modalContent){
                if(select2Destroy){ currentTarget.empty(); }
                ...
                modalContent.find(targetElement).select2({
                    ...
                    vmTab2.search.departments = newData;
                }).on("select2:unselect", function(e){
                    let { search } = vmTab2;
                    const data = e.params.data;
...
$(document).on('click', '#advance-search', function(){
    ...
    if (typeof modalAdvance !== "undefined" && modalAdvance.length == 1) {
        ...
        vmTab1.setDepartmentSelect2('#department', tempModalContent, true);
