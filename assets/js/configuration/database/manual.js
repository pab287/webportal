var search_val = "";
var tblManualBackup = $("#manual-backup").DataTable({
    dom: '<"toolbar">frtlip',
	serverSide: true,
    processing: true,
    ajax: {
    ...
    columnDefs: [
        {
            ...
            targets: -1,
            orderable: false,
            render: function ( data, type, row, meta ) { return itemDatatableActions(row.id, row.filepath); },
        }
    ]
});
...
$.validate({
    ...
    onSuccess : function(form) {
            $.ajax({
                url: baseUrl("configuration/save_manual"),
                type: "POST",
                dataType: "json",
    ...
    $.validate({
        ...
        onSuccess : function(form) {
                $.ajax({
                    ...
                    success: function(data){
                        if(data){
                            $('#database_name').append("").trigger('change');  
