    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function (key, value) {
                ...
                const dlgNewOption = new Option(value.sub_cat_desc, value.sub_cat_id, false, isSame);
                    ...
                    .then((resizedImage) => {
                        ...
                        reader.onloadend = function () {
                            ...
                            const image = "<div class='images-preview-container__wrapper d-flex flex-column'>" +
                                "                   <a data-lightbox='roadtrip' data-title='" + name + "' href='" + base64data + "'>" +
                                ...
                                "                               data-skin='dark' data-delay='{\"show\": 300}' data-placement='bottom'>" +
                        ...
                        if (response.fail_uploads.length) {
                            ...
                            if(response.status == 'archived'){
                                ...
                                setTimeout(() => {
                                    ...
                                    window.location.assign(baseUrl("ams/assets/asset_components_masterfile"));
                                }, 1500);
                            }else{
                                toastr.success(response.message, "Successfully updated.", 50000);
                                $('#confirm-update-fix-asset').modal("hide");
                                setTimeout(() => {
                                    // window.location.assign(baseUrl("ams/assets/fixed_masterfile"));
                                    window.location.assign(baseUrl("ams/assets/edit_asset_component/" + response.id));
                                }, 1500);
                            }
                        }
...
function saveComponentToAsset(_id, el, isMother) {
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                $("#tab-components-table").DataTable().ajax.reload();
                toastr.success(response.message, "Component added to asset.", 10000);
            } else {
                toastr.error(response.message, "Error", 10000);
            }
        }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                tblAllComponentList.ajax.reload();
                $(el).resetForm();
                toastr.success(response.message, "Component Re-included", 10000);
            } else {
                toastr.error(response.message, "Error", 10000);
            }
        }
...
function edit_generate_asset_comp_name(){
    var brand_name = $("#update_comp_asset_brand").val().toLowerCase();
    var model_name = $("#update_comp_asset_model").val().toLowerCase();
    var name = "";
    
    if((brand_name == "n/a" || brand_name == "none") && (model_name == "n/a" || model_name == "none")){
        name = "";
        $("#update_comp_asset_name").attr('readonly', false);
    ...
    $("#update_comp_asset_name").val(name);
}    
