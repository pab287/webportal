...
function formReset() {
    resetSelect2();
    $("#frm-new-asset-component").resetForm();
}
...
function saveNewAssetComponent() {
    const form = $("#frm-new-asset-component");
    ...
    asyncAppendImageToFormData(images, formData)
        .then(() => {
            ...
            $.ajax({
                url: baseUrl("ams/assets/save_new_asset/1"),
                type: "POST",
                dataType: "JSON",
                data: formData,
                processData: false,
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        if (response.fail_uploads.length) {
                            ...
                            toastr.success(response.message, "Notification", { timeOut: 50000 });
...
function generate_asset_comp_name(){
    var brand_name = $("#comp_asset_brand").val().toLowerCase();
    var model_name = $("#comp_asset_model").val().toLowerCase();
    var name = "";
    
    if((brand_name == "n/a" || brand_name == "none") && (model_name == "n/a" || model_name == "none")){
        name = "";
        $("#comp_asset_name").attr('readonly', false);
