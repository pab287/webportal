...
$("#frm-new-vehicle").on("submit", function (e) {
    e.preventDefault();

    if ($(this).isValid()) {
        $('#confirm-save-new-vehicle').modal("show");
    }
});
...
function saveNewVehicle() {
    const form = $("#frm-new-vehicle");
    const formData = new FormData(form[0]);
    const mode = $("#confirm-save-new-vehicle").attr("data-mode");
    ...
    asyncAppendImageToFormData(images, formData)
        .then(() => {
            ...
            const generateCount = $("#frm-generate-multiple-vehicle").find("[name='count']").val();
            const saveTimes = (mode === "multiple") ? generateCount : 1;

            let ctr = 0;
            for (let i = 0; i < saveTimes; i++) {
                $.ajax({
                    url: baseUrl("ams/vehicles/save_new_vehicle"),
                    type: "POST",
                    dataType: "JSON",
                    data: formData,
                    processData: false,
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            if (response.fail_uploads.length) {
                                ...
                                toastr.success(response.message, "Vehicle saved.", 50000);
                                setTimeout(() => {
                            ...
                            if (mode === "multiple") {
                                if (ctr === parseInt(saveTimes)) {
                                    $("#frm-generate-multiple-vehicle").find("[name='count']").val("1");
                                    ...
                                    setTimeout(() => {
                                        ...
                                        window.location.assign(baseUrl("ams/vehicles/new_vehicle"));
                                    }, 1500);
                                }
                            }
                        } else {
                        ...
                        $('#confirm-save-new-vehicle').modal("hide");
                    }
                });
...
function generate_vehicle_name(){
    var brand_name = $("#vehicle_manufacturer").val().toLowerCase();
    var model_name = $("#vehicle_model").val().toLowerCase();
    var name = "";
    
    if((brand_name == "n/a" || brand_name == "none") && (model_name == "n/a" || model_name == "none")){
        name = "";
        $("#vehicle_name").attr('readonly', false);
...
function openGenerateMulipleModal() {
    if ($("#frm-new-vehicle").isValid()) {
        $("#generate-multiple-vehicle-modal").modal("show");
    }
}

$("#generate-multiple-vehicle-modal")
    .on("shown.bs.modal", function () {
        $(this).find("input").focus();
    });

$("#frm-generate-multiple-vehicle")
    .on("submit", function (e) {
        e.preventDefault();
        if ($(this).isValid()) {
            $('#confirm-save-new-vehicle').attr("data-mode", "multiple");
            $('#confirm-save-new-vehicle').modal("show");
        }
    });
