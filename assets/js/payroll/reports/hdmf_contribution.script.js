...
let tempColumns = [
    { data: "pagibig_no", width: "15%", className: "print-size-10" },
    ...
    { data: "employee_name", width: "*", className: "print-size-auto", render: function(data, meta, row){
        const tempData = { name: data, rows: row.ps_data };
    ...
    {
        data: "hdmf_ee", width: "15%", className: "print-size-8 text-right", render: function (data, meta, row) {
            return numberFormat(data);
        }
    },
    {
        data: "hdmf_er", width: "15%", className: "print-size-8 text-right", render: function (data, meta, row) {
            return numberFormat(data);
        }
    },
    {
        data: null, width: "15%", className: "print-size-8 text-right", render: function (data, meta, row) {
            let tempTotal = 0;
            tempTotal = parseFloat(row.hdmf_ee) + parseFloat(row.hdmf_er);
...
let dtRemittances = $('#tbl-hdmf_remittances').DataTable({
    ...
    buttons: [
        {
            ...
            title: function () {
                ...
                return `<div class="m--regular-font-size-lg1">HDMF CONTRIBUTION</div>
                        <div class="m--regular-font-size-sm1 mt-1">COMPANY : ${_tempFilter.companies}</div>
                ...
                if (globalPrintableSignatory.length > 0) {
                    $.each(globalPrintableSignatory, function (i, v) {
                        ...
                        if (tempLabel && v.is_active == true) {
                            let tempCell = `<div style='display: inline-block; position: relative; width: 33.33%; margin-top: 30px;'>
    ...
    /*** ajax: {
        url: baseUrl('payroll/reports/get_hdmf_remittance_report_request'),
        type: 'POST',
        dataType: 'JSON',
        data: function (d) {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json.response) {
                _clearTable = false;
                _tempIds = json.data;
                dtRemittances.clear().rows.add(json.data).draw(false);

                _tempFilter = Object.assign({}, json.filters);
                psGenerateReportModal.modal("hide");
                toastr.success(json.toastr_msg, "Filtered HDMF Contribution");
            } else {
                _clearTable = true;
                toastr.error(json.toastr_msg, "Filtered HDMF Contribution");
            }
...
var vmPsData = new Vue({
    ...
    methods: {
        formattedDate: function(date){
            ...
            return tempDate;
        }, formattedAdjustments(adjustment, type='hdmf'){
