var search_val = "";
var temp_images = [];
var tblBlacklist = $("#table-blacklisted")
    .DataTable({
        dom: 'rtlip',
        serverSide: true,
        processing: true,
        ...
        searching: false,
        columns: [
          {data: "blacklisted_dt",visible: false},
            {
                data: "name",
                width: "13%"
            },
            ...
            { 
                extend: 'csv',
                title: 'CRS BLACKLISTED - RESUME REPORT',
                exportOptions: {
    ...
    function itemDatatableActions($id) {
        if ($id) {
            ...
            _actionButton += "<a style='width: auto; cursor: pointer;' class='dropdown-item' onclick='view_blacklist(" + $id + ")'><i class='la la-eye'></i>View</a>";
            _actionButton += "<a style='width: auto; cursor: pointer;' class='dropdown-item' onclick='repending(" + $id + ")'><i class='la la-pencil-square'></i>Re Pending</a>";
            _actionButton += "<a style='width: auto; cursor: pointer;' class='dropdown-item' onclick='blacklist_delete(" + $id + ")'><i class='la la-trash'></i>Delete</a>";
            _actionButton += " </div>";
            _actionButton += "</div>";
    ...
    $("#ExportExcel").on("click", function(e) {
        e.preventDefault();
        tblBlacklist.button( '.buttons-excel' ).trigger();
    });
    
    $("#ExportCSV").on("click", function(e) {
        e.preventDefault();
        tblBlacklist.button( '.buttons-csv' ).trigger();
    });
    
    $("#ExportPDF").on("click", function(e) {
        e.preventDefault();
        tblBlacklist.button( '.buttons-pdf' ).trigger();
    });

    function view_blacklist($id){
...
function initDatatable(columns, fields, order_field, order_by, criteria) {
    ...
    dtReport = $('#table-generated-report')
        .DataTable({
            ...
            buttons: [
                {
                    ...
                    action: function (e, dt, node, config) {
                        const self = this;
                        getExportData(e, dt, node, config, self, baseUrl('crs/generate_employee_report_blacklisted/1'), 'excelHtml5')
                            .then(() => {
                ...
                },
                {
                    ...
                    action: function (e, dt, node, config) {
                        const self = this;
                        getExportData(e, dt, node, config, self, baseUrl('crs/generate_employee_report_blacklisted/1'), 'pdfHtml5')
                            .then(() => {
                                dropdown.removeClass("m-btn--custom m-loader m-loader--light m-loader--left");
                            });
                    }
                },
                {
                    ...
                    action: function (e, dt, node, config) {
                        const self = this;
                        getExportData(e, dt, node, config, self, baseUrl('crs/generate_employee_report_blacklisted/1'), 'print')
                            .then(() => {
                                dropdown.removeClass("m-btn--custom m-loader m-loader--light m-loader--left");
                            });
                    }
            ...
            ordering: false,
            ajax: {
                url: baseUrl('crs/generate_employee_report_blacklisted'),
                type: 'post',
                dataType: 'json',
                data: function (d) {
                    d.csrf_token = _csrf_hash;
