...
function loadEmployees(employee_status = "All") {
    if (typeof tableEmployeeList !== "undefined") {
        ...
        dtEmployee = tableEmployeeList.DataTable({
            ...
            destroy: true,
            ajax: {
                url: baseUrl("hris/masterfile/tempget_employee_datatable_request/" + employee_status),
                type: "post",
                dataType: "json",
                data: function (d) {
    ...
    if (typeof tableOffenses !== "undefined") {
        var dtOffenses = tableOffenses.DataTable({
            ...
            columns: [
                ...
                { data: "offcom_date", title: "Date" },
                { data: "offcom_nature", title: "Nature" },
                { data: "offcom_action", title: "Action Taken" },
                { data: null, title: "Action", width: "8%", className: "text-center" }
            ],
            columnDefs: [
                {
        ...
        var dtDocuments = tableDocuments.DataTable({
            ...
            initComplete: function () {
                $(".dt-toolbar_documents").append(
                    "<button type='button' class='btn btn-sm btn-success mb-2 btnNew btnAddDocuments'><i class='la la-plus mr-1'></i>New</button>"
                );
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.response) {
                        ...
                        $("#employement_information i").remove();
                    } else {
                        toastr.error(
                            json.toastr_msg,
                            "Error updating employee data!",
