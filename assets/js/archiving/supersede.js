...
var tblMemo = $("#table-memo").DataTable({
    ...
    processing: true,
    ajax: {
		url: baseUrl("archiving/get_supersede_collection/"),
		type: "post",
        dataType: "json",
       data: function(d){
    ...
    columnDefs: [
        
        {
            ...
            render: function ( data, type, row, meta ) { return itemDatatableActions(row.id,row.is_fav); },
        }
...
function itemDatatableActions($id,$fav){
    ...
    _actionButton +="<a class='dropdown-item' onclick='restore("+$id+")'><i class='la la-ban'></i>Restore</a>";
    _actionButton +=" </div>";
    _actionButton +="</div>";
    _actionButton +="</span>";
...
function restore($id)
{
      ...
      $.ajax({
        url : baseUrl("archiving/unsupersede_memo/") + $id,
        type: "POST",
        dataType: "JSON",
        data:  { csrf_token: _csrf_hash },
        success: function(data)
